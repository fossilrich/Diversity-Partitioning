...
mydata <- read.csv("//naturkundemuseum-berlin.de/MuseumDFSRoot/Benutzer/richard.hofmann/Desktop/PNAS_RESUB/Vetted/Permian3-vetted.csv", 
                   row.names = NULL, sep=";")
#mydata <- read.csv("C:/Users/fossi/Documents/pRojects/MatrixNew/Vetted/Permian3-vetted.csv", 
#                   row.names = NULL, sep=";")
...
#  Ab.Env.marg <- c(Ab.Env.marg, (abund.envo.perc[1,5]))
  ...
  if(i==1){
    Alpha.Perm3 <- data.frame(Alpha)
  }else{Alpha.Perm3 <- cbind(Alpha.Perm3, Alpha)}
  
  if(i==1){
    BetaW.Perm3 <- data.frame(BetaW)
  }else{BetaW.Perm3 <- cbind(BetaW.Perm3, BetaW)}
  
  if(i==1){
    BetaJ.Perm3 <- data.frame(BetaJ)
  }else{BetaJ.Perm3 <- cbind(BetaJ.Perm3, BetaJ)}
  
  if(i==1){
    BetaS.Perm3 <- data.frame(BetaS)
  }else{BetaS.Perm3 <- cbind(BetaS.Perm3, BetaS)}
  
  if(i==1){
    Gamma.Perm3 <- data.frame(Gamma)
  }else{Gamma.Perm3 <- cbind(Gamma.Perm3, Gamma)}
  
  if(i==1){
    BetaSim.Perm3 <- data.frame(BetaSim)
  }else{BetaSim.Perm3 <- cbind(BetaSim.Perm3, BetaSim)}
  ...
  BetaSimForm <- c(BetaSimForm, mean(BetaSim))
...
Perm3Dat <- cbind(Age, AlphaForm, BetaSimForm,
...
write.csv(Perm3Dat, file = "08_Perm3Dat.csv")
