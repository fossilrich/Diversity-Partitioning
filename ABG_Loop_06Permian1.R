...
mydata <- read.csv("//naturkundemuseum-berlin.de/MuseumDFSRoot/Benutzer/richard.hofmann/Desktop/PNAS_RESUB/Vetted/Permian1-vetted.csv", 
                   row.names = NULL, sep=";")
#mydata <- read.csv("C:/Users/fossi/Documents/pRojects/MatrixNew/Vetted/Devonian-vetted.csv", 
#                   row.names = NULL, sep=";")
...
levels(mydata$collections.environment)[levels(mydata$collections.environment)%in%c("interdistributary bay", "estuary/bay", "delta plain", "delta front", "lagoonal/restricted shallow subtidal","paralic indet.","lagoonal", "peritidal", "prodelta","marginal marine indet.", "deltaic indet.")] <- "marginal marine"
...
#creating dataframes for abundances of environments in each formation
  ...
  Ab.Tril <- c(Ab.Tril, (abund.class[1,4]))
 ...
 # Ab.Env.bas <- c(Ab.Env.bas, (abund.envo.perc[1,2]))
  Ab.Env.shall <- c(Ab.Env.shall, (abund.envo.perc[1,2]))
  Ab.Env.deep <- c(Ab.Env.deep, (abund.envo.perc[1,4]))
  Ab.Env.marg <- c(Ab.Env.marg, (abund.envo.perc[1,3]))
  Ab.Env.reef <- c(Ab.Env.reef, (abund.envo.perc[1,5]))
  Ab.Env.slope <- c(Ab.Env.slope, (abund.envo.perc[1,6]))
  
   ####now we collect all diversities from each run for further processing
  if(i==1){
    Alpha.Perm1 <- data.frame(Alpha)
    }else{Alpha.Perm1 <- cbind(Alpha.Perm1, Alpha)}

  if(i==1){
    BetaW.Perm1 <- data.frame(BetaW)
    }else{BetaW.Perm1 <- cbind(BetaW.Perm1, BetaW)}
  
 if(i==1){
    BetaJ.Perm1 <- data.frame(BetaJ)
    }else{BetaJ.Perm1 <- cbind(BetaJ.Perm1, BetaJ)}
  
  if(i==1){
    BetaS.Perm1 <- data.frame(BetaS)
    }else{BetaS.Perm1 <- cbind(BetaS.Perm1, BetaS)}
  
 if(i==1){
    Gamma.Perm1 <- data.frame(Gamma)
    }else{Gamma.Perm1 <- cbind(Gamma.Perm1, Gamma)}
  
  ####NEWBETA_4####
  if(i==1){
    BetaSim.Perm1 <- data.frame(BetaSim)
  }else{BetaSim.Perm1 <- cbind(BetaSim.Perm1, BetaSim)}
...
Perm1Dat <- cbind(Age, AlphaForm, BetaSimForm,####NEWBETA_6####
                ...
                Plate, Ab.Tril, Ab.Brach, Ab.Moll, Ab.Echi)

View(Perm1Dat)
write.csv(Perm1Dat, file = "06_Perm1Dat.csv")