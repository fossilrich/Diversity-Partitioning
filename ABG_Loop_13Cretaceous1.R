...
mydata <- read.csv("//naturkundemuseum-berlin.de/MuseumDFSRoot/Benutzer/richard.hofmann/Desktop/PNAS_RESUB/Vetted/Cretaceous1-vetted.csv", 
                   row.names = NULL, sep=";")
#mydata <- read.csv("C:/Users/fossi/Documents/pRojects/MatrixNew/Vetted/Cretaceous1-vetted.csv", 
#                   row.names = NULL, sep=";")
...
for(i in 1:length(formation.name)){
  ...
  if(i==1){
    Alpha.Creta1 <- data.frame(Alpha)
  }else{Alpha.Creta1 <- cbind(Alpha.Creta1, Alpha)}
  
  if(i==1){
    BetaW.Creta1 <- data.frame(BetaW)
  }else{BetaW.Creta1 <- cbind(BetaW.Creta1, BetaW)}
  
  if(i==1){
    BetaJ.Creta1 <- data.frame(BetaJ)
  }else{BetaJ.Creta1 <- cbind(BetaJ.Creta1, BetaJ)}
  
  if(i==1){
    BetaS.Creta1 <- data.frame(BetaS)
  }else{BetaS.Creta1 <- cbind(BetaS.Creta1, BetaS)}
  
  if(i==1){
    Gamma.Creta1 <- data.frame(Gamma)
  }else{Gamma.Creta1 <- cbind(Gamma.Creta1, Gamma)}
  
  ####NEWBETA_4####
  if(i==1){
    BetaSim.Creta1 <- data.frame(BetaSim)
  }else{BetaSim.Creta1 <- cbind(BetaSim.Creta1, BetaSim)}
...
Creta1Dat <- cbind(Age, AlphaForm, BetaSimForm,####NEWBETA_6####
                  BetaWForm, BetaSForm, BetaJForm, GammaForm, CollpF, 
                  Duration, RefForm, Formation, Environments, Ab.Env.unkn, Ab.Env.marg,
                  Ab.Env.shall, Ab.Env.deep, Ab.Env.bas, #Ab.Env.reef,  Ab.Env.slope, 
                  Plate, Ab.Echi, Ab.Brach, Ab.Moll)

write.csv(Creta1Dat, file = "13_Creta1Dat.csv")