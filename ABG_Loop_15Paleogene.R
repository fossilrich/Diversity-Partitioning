...
mydata <- read.csv("//naturkundemuseum-berlin.de/MuseumDFSRoot/Benutzer/richard.hofmann/Desktop/PNAS_RESUB/Vetted/Paleogene-vetted.csv", 
                   row.names = NULL, sep=";")
#mydata <- read.csv("C:/Users/fossi/Documents/pRojects/MatrixNew/Vetted/Paleogene-vetted.csv", 
#                 row.names = NULL, sep=";")
...
#  Ab.Env.slope <- c(Ab.Env.slope, (abund.envo.perc[1,7]))
  ...
  if(i==1){
    Alpha.Paleog <- data.frame(Alpha)
  }else{Alpha.Paleog <- cbind(Alpha.Paleog, Alpha)}
  
  if(i==1){
    BetaW.Paleog <- data.frame(BetaW)
  }else{BetaW.Paleog <- cbind(BetaW.Paleog, BetaW)}
  
  if(i==1){
    BetaJ.Paleog <- data.frame(BetaJ)
  }else{BetaJ.Paleog <- cbind(BetaJ.Paleog, BetaJ)}
  
  if(i==1){
    BetaS.Paleog <- data.frame(BetaS)
  }else{BetaS.Paleog <- cbind(BetaS.Paleog, BetaS)}
  
  if(i==1){
    Gamma.Paleog <- data.frame(Gamma)
  }else{Gamma.Paleog <- cbind(Gamma.Paleog, Gamma)}
  
  ####NEWBETA_4####
  if(i==1){
    BetaSim.Paleog <- data.frame(BetaSim)
  }else{BetaSim.Paleog <- cbind(BetaSim.Paleog, BetaSim)}
...
PaleogDat <- cbind(Age, AlphaForm, BetaSimForm,
...
write.csv(PaleogDat, file = "15_PaleogDat.csv")
