...
mydata <- read.csv("//naturkundemuseum-berlin.de/MuseumDFSRoot/Benutzer/richard.hofmann/Desktop/PNAS_RESUB/Vetted/Devonian-vetted.csv", 
                   row.names = NULL, sep=";")
...
#creating dataframes for abundances of environments in each formation
  ...
  if(i==1){
    Alpha.Dev <- data.frame(Alpha)
    }else{Alpha.Dev <- cbind(Alpha.Dev, Alpha)}

  if(i==1){
    BetaW.Dev <- data.frame(BetaW)
    }else{BetaW.Dev <- cbind(BetaW.Dev, BetaW)}
  
 if(i==1){
    BetaJ.Dev <- data.frame(BetaJ)
    }else{BetaJ.Dev <- cbind(BetaJ.Dev, BetaJ)}
  
  if(i==1){
    BetaS.Dev <- data.frame(BetaS)
    }else{BetaS.Dev <- cbind(BetaS.Dev, BetaS)}
  
 if(i==1){
    Gamma.Dev <- data.frame(Gamma)
    }else{Gamma.Dev <- cbind(Gamma.Dev, Gamma)}
  
  ####NEWBETA_4####
  if(i==1){
    BetaSim.Dev <- data.frame(BetaSim)
  }else{BetaSim.Dev <- cbind(BetaSim.Dev, BetaSim)}
...
DevDat <- cbind(Age, AlphaForm, BetaSimForm,####NEWBETA_6####
...
write.csv(DevDat, file = "04_DevDat.csv")