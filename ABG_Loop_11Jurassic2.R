...
mydata <- read.csv("//naturkundemuseum-berlin.de/MuseumDFSRoot/Benutzer/richard.hofmann/Desktop/PNAS_RESUB/Vetted/Jurassic2-vetted.csv", 
                   row.names = NULL, sep=";")
#mydata <- read.csv("C:/Users/fossi/Documents/pRojects/MatrixNew/Vetted/Jurassic2-vetted.csv", 
#                   row.names = NULL, sep=";")
...
#  Ab.Env.bas <- c(Ab.Env.bas, (abund.envo.perc[1,2]))
  ...
  Ab.Env.deep <- c(Ab.Env.deep, (abund.envo.perc[1,2]))
...
#  Ab.Env.slope <- c(Ab.Env.slope, (abund.envo.perc[1,7]))
  ...
  if(i==1){
    Alpha.Jura2 <- data.frame(Alpha)
  }else{Alpha.Jura2 <- cbind(Alpha.Jura2, Alpha)}
  
  if(i==1){
    BetaW.Jura2 <- data.frame(BetaW)
  }else{BetaW.Jura2 <- cbind(BetaW.Jura2, BetaW)}
  
  if(i==1){
    BetaJ.Jura2 <- data.frame(BetaJ)
  }else{BetaJ.Jura2 <- cbind(BetaJ.Jura2, BetaJ)}
  
  if(i==1){
    BetaS.Jura2 <- data.frame(BetaS)
  }else{BetaS.Jura2 <- cbind(BetaS.Jura2, BetaS)}
  
  if(i==1){
    Gamma.Jura2 <- data.frame(Gamma)
  }else{Gamma.Jura2 <- cbind(Gamma.Jura2, Gamma)}
  
  ####NEWBETA_4####
  if(i==1){
    BetaSim.Jura2 <- data.frame(BetaSim)
  }else{BetaSim.Jura2 <- cbind(BetaSim.Jura2, BetaSim)}
...
Jura2Dat <- cbind(Age, AlphaForm, BetaSimForm,####NEWBETA_6####
...
write.csv(Jura2Dat, file = "11_Jura2Dat.csv")
