...
mydata <- read.csv("//naturkundemuseum-berlin.de/MuseumDFSRoot/Benutzer/richard.hofmann/Desktop/PNAS_RESUB/Vetted/Triassic-vetted.csv", 
                   row.names = NULL, sep=";")
#mydata <- read.csv("C:/Users/fossi/Documents/pRojects/MatrixNew/Vetted/Triassic-vetted.csv", 
#                   row.names = NULL, sep=";")
...
levels(mydata$collections.environment)[levels(mydata$collections.environment)%in%c("reef", "reef, buildup or bioherm","platform/shelf-margin reef", "buildup or bioherm", "perireef or subreef", "intrashelf/intraplatform reef")] <- "reef"
...
#creating dataframes for abundances of environments in each formation
  ...
  Ab.Env.marg <- c(Ab.Env.marg, (abund.envo.perc[1,6]))
  ...
  if(i==1){
    Alpha.Trias <- data.frame(Alpha)
    }else{Alpha.Trias <- cbind(Alpha.Trias, Alpha)}

  if(i==1){4
    BetaW.Trias <- data.frame(BetaW)
    }else{BetaW.Trias <- cbind(BetaW.Trias, BetaW)}
  
 if(i==1){
    BetaJ.Trias <- data.frame(BetaJ)
    }else{BetaJ.Trias <- cbind(BetaJ.Trias, BetaJ)}
  
  if(i==1){
    BetaS.Trias <- data.frame(BetaS)
    }else{BetaS.Trias <- cbind(BetaS.Trias, BetaS)}
  
 if(i==1){
    Gamma.Trias <- data.frame(Gamma)
    }else{Gamma.Trias <- cbind(Gamma.Trias, Gamma)}
  
  ####NEWBETA_4####
  if(i==1){
    BetaSim.Trias <- data.frame(BetaSim)
  }else{BetaSim.Trias <- cbind(BetaSim.Trias, BetaSim)}
...
TriasDat <- cbind(Age, AlphaForm, BetaSimForm,####NEWBETA_6####
                ...
                Plate, Ab.Brach, Ab.Moll, Ab.Echi)

write.csv(TriasDat, file = "09_TriasDat.csv")