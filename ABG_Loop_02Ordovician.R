...
mydata <- read.csv("//naturkundemuseum-berlin.de/MuseumDFSRoot/Benutzer/richard.hofmann/Desktop/PNAS_RESUB/Vetted/Ordovician-vetted.csv", 
                   row.names = NULL, sep=";")
#mydata <- read.csv("C:/Users/fossi/Documents/pRojects/MatrixNew/Vetted/Cambrian-vetted.csv", 
#                   row.names = NULL, sep=";")
...
#creating dataframes for abundances of environments in each formation
  ...
  if(i==1){
    Alpha.Ord <- data.frame(Alpha)
    }else{Alpha.Ord <- cbind(Alpha.Ord, Alpha)}

  if(i==1){
    BetaW.Ord <- data.frame(BetaW)
    }else{BetaW.Ord <- cbind(BetaW.Ord, BetaW)}
  
 if(i==1){
    BetaJ.Ord <- data.frame(BetaJ)
    }else{BetaJ.Ord <- cbind(BetaJ.Ord, BetaJ)}
  
  if(i==1){
    BetaS.Ord <- data.frame(BetaS)
    }else{BetaS.Ord <- cbind(BetaS.Ord, BetaS)}
  
 if(i==1){
    Gamma.Ord <- data.frame(Gamma)
    }else{Gamma.Ord <- cbind(Gamma.Ord, Gamma)}
  
  if(i==1){
    BetaSim.Ord <- data.frame(BetaSim)
  }else{BetaSim.Ord <- cbind(BetaSim.Ord, BetaSim)}
...
OrdDat <- cbind(Age, AlphaForm, BetaSimForm,####NEWBETA_6####
...
write.csv(OrdDat, file = "02_OrdDat.csv")