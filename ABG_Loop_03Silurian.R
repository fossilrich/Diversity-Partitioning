...
mydata <- read.csv("//naturkundemuseum-berlin.de/MuseumDFSRoot/Benutzer/richard.hofmann/Desktop/PNAS_RESUB/Vetted/Silurian-vetted.csv", 
                   row.names = NULL, sep=";")
#mydata <- read.csv("C:/Users/fossi/Documents/pRojects/MatrixNew/Vetted/Silurian-vetted.csv", 
#                   row.names = NULL, sep=";")
...
#creating dataframes for abundances of environments in each formation
  ...
  if(i==1){
    Alpha.Sil <- data.frame(Alpha)
    }else{Alpha.Sil <- cbind(Alpha.Sil, Alpha)}

  if(i==1){
    BetaW.Sil <- data.frame(BetaW)
    }else{BetaW.Sil <- cbind(BetaW.Sil, BetaW)}
  
 if(i==1){
    BetaJ.Sil <- data.frame(BetaJ)
    }else{BetaJ.Sil <- cbind(BetaJ.Sil, BetaJ)}
  
  if(i==1){
    BetaS.Sil <- data.frame(BetaS)
    }else{BetaS.Sil <- cbind(BetaS.Sil, BetaS)}
  
 if(i==1){
    Gamma.Sil <- data.frame(Gamma)
    }else{Gamma.Sil <- cbind(Gamma.Sil, Gamma)}
  
  ####NEWBETA_4####
  if(i==1){
    BetaSim.Sil <- data.frame(BetaSim)
  }else{BetaSim.Sil <- cbind(BetaSim.Sil, BetaSim)}
...
SilDat <- cbind(Age, AlphaForm, BetaSimForm,####NEWBETA_6####
...
write.csv(SilDat, file = "03_SilDat.csv")