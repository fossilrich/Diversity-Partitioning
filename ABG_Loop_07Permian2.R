...
mydata <- read.csv("//naturkundemuseum-berlin.de/MuseumDFSRoot/Benutzer/richard.hofmann/Desktop/PNAS_RESUB/Vetted/Permian2-vetted.csv", 
                   row.names = NULL, sep=";")
#mydata <- read.csv("C:/Users/fossi/Documents/pRojects/MatrixNew/Vetted/Permian2-vetted.csv", 
#                   row.names = NULL, sep=";")
...
levels(mydata$collections.environment)[levels(mydata$collections.environment)%in%c("slope/ramp reef", "reef", "reef, buildup or bioherm","platform/shelf-margin reef", "buildup or bioherm", "perireef or subreef")] <- "reef"
...
for(i in 1:length(formation.name)){
 ...
 # Ab.Env.bas <- c(Ab.Env.bas, (abund.envo.perc[1,2])) # no basin
  ...
  if(i==1){
    Alpha.Perm2 <- data.frame(Alpha)
  }else{Alpha.Perm2 <- cbind(Alpha.Perm2, Alpha)}
  
  if(i==1){
    BetaW.Perm2 <- data.frame(BetaW)
  }else{BetaW.Perm2 <- cbind(BetaW.Perm2, BetaW)}
  
  if(i==1){
    BetaJ.Perm2 <- data.frame(BetaJ)
  }else{BetaJ.Perm2 <- cbind(BetaJ.Perm2, BetaJ)}
  
  if(i==1){
    BetaS.Perm2 <- data.frame(BetaS)
  }else{BetaS.Perm2 <- cbind(BetaS.Perm2, BetaS)}
  
  if(i==1){
    Gamma.Perm2 <- data.frame(Gamma)
  }else{Gamma.Perm2 <- cbind(Gamma.Perm2, Gamma)}
  
  ####NEWBETA_4####
  if(i==1){
    BetaSim.Perm2 <- data.frame(BetaSim)
  }else{BetaSim.Perm2 <- cbind(BetaSim.Perm2, BetaSim)}
...
Perm2Dat <- cbind(Age, AlphaForm, BetaSimForm,
...
write.csv(Perm2Dat, file = "07_Perm2Dat.csv")
