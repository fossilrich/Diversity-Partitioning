...
mydata <- read.csv("//naturkundemuseum-berlin.de/MuseumDFSRoot/Benutzer/richard.hofmann/Desktop/PNAS_RESUB/Vetted/Neogene2-vetted.csv", 
                   row.names = NULL, sep=";")
#mydata <- read.csv("C:/Users/fossi/Documents/pRojects/MatrixNew/Vetted/Neogene2-vetted.csv", 
#                   row.names = NULL, sep=";")
...
levels(mydata$collections.environment)[levels(mydata$collections.environment)%in%c(" buildup or bioherm", "intrashelf/intraplatform reef","slope/ramp reef", "reef", "reef, buildup or bioherm","platform/shelf-margin reef", "buildup or bioherm", "perireef or subreef")] <- "reef"
...
#  Ab.Env.bas <- c(Ab.Env.bas, (abund.envo.perc[1,2]))
  ...
  Ab.Env.deep <- c(Ab.Env.deep, (abund.envo.perc[1,5]))
  Ab.Env.marg <- c(Ab.Env.marg, (abund.envo.perc[1,4]))
  Ab.Env.reef <- c(Ab.Env.reef, (abund.envo.perc[1,2]))
#  Ab.Env.slope <- c(Ab.Env.slope, (abund.envo.perc[1,7]))
  
  ####now we collect all diversities from each run for further processing
  if(i==1){
    Alpha.Neog2 <- data.frame(Alpha)
  }else{Alpha.Neog2 <- cbind(Alpha.Neog2, Alpha)}
  
  if(i==1){
    BetaW.Neog2 <- data.frame(BetaW)
  }else{BetaW.Neog2 <- cbind(BetaW.Neog2, BetaW)}
  
  if(i==1){
    BetaJ.Neog2 <- data.frame(BetaJ)
  }else{BetaJ.Neog2 <- cbind(BetaJ.Neog2, BetaJ)}
  
  if(i==1){
    BetaS.Neog2 <- data.frame(BetaS)
  }else{BetaS.Neog2 <- cbind(BetaS.Neog2, BetaS)}
  
  if(i==1){
    Gamma.Neog2 <- data.frame(Gamma)
  }else{Gamma.Neog2 <- cbind(Gamma.Neog2, Gamma)}
  
  ####NEWBETA_4####
  if(i==1){
    BetaSim.Neog2 <- data.frame(BetaSim)
  }else{BetaSim.Neog2  <- cbind(BetaSim.Neog2 , BetaSim)}
...
Neog2Dat <- cbind(Age, AlphaForm, BetaSimForm,####NEWBETA_6####
...
write.csv(Neog2Dat, file = "17_Neog2Dat.csv")