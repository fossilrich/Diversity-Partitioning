...
mydata <- read.csv("//naturkundemuseum-berlin.de/MuseumDFSRoot/Benutzer/richard.hofmann/Desktop/PNAS_RESUB/Vetted/Cretaceous2-vetted.csv", 
                   row.names = NULL, sep=";")
#mydata <- read.csv("C:/Users/fossi/Documents/pRojects/MatrixNew/Vetted/Cretaceous2-vetted.csv", 
#                   row.names = NULL, sep=";")
...
for(i in 1:length(formation.name)){
  ...
  if(i==1){
    Alpha.Creta2 <- data.frame(Alpha)
  }else{Alpha.Creta2 <- cbind(Alpha.Creta2, Alpha)}
  
  if(i==1){
    BetaW.Creta2 <- data.frame(BetaW)
  }else{BetaW.Creta2 <- cbind(BetaW.Creta2, BetaW)}
  
  if(i==1){
    BetaJ.Creta2 <- data.frame(BetaJ)
  }else{BetaJ.Creta2 <- cbind(BetaJ.Creta2, BetaJ)}
  
  if(i==1){
    BetaS.Creta2 <- data.frame(BetaS)
  }else{BetaS.Creta2 <- cbind(BetaS.Creta2, BetaS)}
  
  if(i==1){
    Gamma.Creta2 <- data.frame(Gamma)
  }else{Gamma.Creta2 <- cbind(Gamma.Creta2, Gamma)}
  
  ####NEWBETA_4####
  if(i==1){
    BetaSim.Creta2 <- data.frame(BetaSim)
  }else{BetaSim.Creta2 <- cbind(BetaSim.Creta2, BetaSim)}
...
Creta2Dat <- cbind(Age, AlphaForm, BetaSimForm,####NEWBETA_6####
                   BetaWForm, BetaSForm, BetaJForm, GammaForm, CollpF, 
                   Duration, RefForm, Formation, Environments, Ab.Env.unkn, Ab.Env.marg,
                   Ab.Env.shall, Ab.Env.reef, Ab.Env.deep, Ab.Env.bas, #Ab.Env.slope, 
                   Plate, Ab.Echi, Ab.Brach, Ab.Moll)

write.csv(Creta2Dat, file = "14_Creta2Dat.csv")
