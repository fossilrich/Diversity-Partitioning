...
mydata <- read.csv("//naturkundemuseum-berlin.de/MuseumDFSRoot/Benutzer/richard.hofmann/Desktop/PNAS_RESUB/Vetted/Carboniferous-vetted.csv", 
                   row.names = NULL, sep=";")
#mydata <- read.csv("C:/Users/fossi/Documents/pRojects/MatrixNew/Vetted/Carboniferous-vetted.csv", 
#                   row.names = NULL, sep=";")
...
#creating dataframes for abundances of environments in each formation
  ...
  for(j in 1:500)
   ...
   # write.csv(NewSubtemp, file = "matrix[i].csv",row.names=FALSE, na="")
    class(NewSubtemp) <- "numeric"
    Multi <- beta.multi(NewSubtemp, index.family="sorensen")
    SimMulti <- Multi[[1]]
  ...
  Ab.Env.shall <- c(Ab.Env.shall, (abund.envo.perc[1,4]))
  Ab.Env.deep <- c(Ab.Env.deep, (abund.envo.perc[1,3]))
  Ab.Env.marg <- c(Ab.Env.marg, (abund.envo.perc[1,2]))
  ...
  if(i==1){
    Alpha.Carb <- data.frame(Alpha)
    }else{Alpha.Carb <- cbind(Alpha.Carb, Alpha)}

  if(i==1){
    BetaW.Carb <- data.frame(BetaW)
    }else{BetaW.Carb <- cbind(BetaW.Carb, BetaW)}
  
 if(i==1){
    BetaJ.Carb <- data.frame(BetaJ)
    }else{BetaJ.Carb <- cbind(BetaJ.Carb, BetaJ)}
  
  if(i==1){
    BetaS.Carb <- data.frame(BetaS)
    }else{BetaS.Carb <- cbind(BetaS.Carb, BetaS)}
  
 if(i==1){
    Gamma.Carb <- data.frame(Gamma)
    }else{Gamma.Carb <- cbind(Gamma.Carb, Gamma)}
  
  if(i==1){
    BetaSim.Carb <- data.frame(BetaSim)
  }else{BetaSim.Carb <- cbind(BetaSim.Carb, BetaSim)}
...
CarbDat <- cbind(Age, AlphaForm, BetaSimForm,
...
write.csv(CarbDat, file = "05_CarbDat.csv")