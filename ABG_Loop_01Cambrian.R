...
mydata <- read.csv("//naturkundemuseum-berlin.de/MuseumDFSRoot/Benutzer/richard.hofmann/Desktop/PNAS_RESUB/Vetted/Cambrian-vetted.csv", 
                   row.names = NULL, sep=";")
...
#creating dataframes for abundances of environments in each formation
 ...
 if(i==1){
  ...
  ####NEWBETA_4####
  if(i==1){
    BetaSim.Cam <- data.frame(BetaSim)
  }else{BetaSim.Cam <- cbind(BetaSim.Cam, BetaSim)}
...
CamDat <- cbind(Age, AlphaForm, BetaSimForm,####NEWBETA_6####
...
write.csv(CamDat, file = "01_CamDat.csv")