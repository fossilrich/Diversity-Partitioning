...
mydata <- read.csv("//naturkundemuseum-berlin.de/MuseumDFSRoot/Benutzer/richard.hofmann/Desktop/PNAS_RESUB/Vetted/Neogene1-vetted.csv", 
                   row.names = NULL, sep=";")
#mydata <- read.csv("C:/Users/fossi/Documents/pRojects/MatrixNew/Vetted/Neogene1-vetted.csv", 
#                   row.names = NULL, sep=";")
...
#  Ab.Env.bas <- c(Ab.Env.bas, (abund.envo.perc[1,2]))
 ...
 # Ab.Env.slope <- c(Ab.Env.slope, (abund.envo.perc[1,7]))
  ...
  if(i==1){
    Alpha.Neog1 <- data.frame(Alpha)
  }else{Alpha.Neog1 <- cbind(Alpha.Neog1, Alpha)}
  
  if(i==1){
    BetaW.Neog1 <- data.frame(BetaW)
  }else{BetaW.Neog1 <- cbind(BetaW.Neog1, BetaW)}
  
  if(i==1){
    BetaJ.Neog1 <- data.frame(BetaJ)
  }else{BetaJ.Neog1 <- cbind(BetaJ.Neog1, BetaJ)}
  
  if(i==1){
    BetaS.Neog1 <- data.frame(BetaS)
  }else{BetaS.Neog1 <- cbind(BetaS.Neog1, BetaS)}
  
  if(i==1){
    Gamma.Neog1 <- data.frame(Gamma)
  }else{Gamma.Neog1 <- cbind(Gamma.Neog1, Gamma)}
  
  ####NEWBETA_4####
  if(i==1){
    BetaSim.Neog1 <- data.frame(BetaSim)
  }else{BetaSim.Neog1 <- cbind(BetaSim.Neog1, BetaSim)}
...
Neog1Dat <- cbind(Age, AlphaForm, BetaSimForm,
                  BetaWForm, BetaSForm, BetaJForm, GammaForm, CollpF, 
                   Duration, RefForm, Formation, Environments, Ab.Env.unkn, Ab.Env.marg,
                   Ab.Env.shall, Ab.Env.deep, #Ab.Env.reef,  Ab.Env.slope, Ab.Env.bas,
                   Plate, Ab.Echi, Ab.Brach, Ab.Moll)

write.csv(Neog1Dat, file = "16_Neog1Dat.csv")