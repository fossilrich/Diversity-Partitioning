...
mydata <- read.csv("//naturkundemuseum-berlin.de/MuseumDFSRoot/Benutzer/richard.hofmann/Desktop/PNAS_RESUB/Vetted/Jurassic3-vetted.csv", 
                   row.names = NULL, sep=";")
#mydata <- read.csv("C:/Users/fossi/Documents/pRojects/MatrixNew/Vetted/Jurassic3-vetted.csv", 
#                   row.names = NULL, sep=";")
...
for(i in 1:length(formation.name)){
 ...
 # Ab.Env.bas <- c(Ab.Env.bas, (abund.envo.perc[1,2]))
  ...
  if(i==1){
    Alpha.Jura3 <- data.frame(Alpha)
  }else{Alpha.Jura3 <- cbind(Alpha.Jura3, Alpha)}
  
  if(i==1){
    BetaW.Jura3 <- data.frame(BetaW)
  }else{BetaW.Jura3 <- cbind(BetaW.Jura3, BetaW)}
  
  if(i==1){
    BetaJ.Jura3 <- data.frame(BetaJ)
  }else{BetaJ.Jura3 <- cbind(BetaJ.Jura3, BetaJ)}
  
  if(i==1){
    BetaS.Jura3 <- data.frame(BetaS)
  }else{BetaS.Jura3 <- cbind(BetaS.Jura3, BetaS)}
  
  if(i==1){
    Gamma.Jura3 <- data.frame(Gamma)
  }else{Gamma.Jura3 <- cbind(Gamma.Jura3, Gamma)}
  
  ####NEWBETA_4####
  if(i==1){
    BetaSim.Jura3 <- data.frame(BetaSim)
  }else{BetaSim.Jura3 <- cbind(BetaSim.Jura3, BetaSim)}
...
Jura3Dat <- cbind(Age, AlphaForm, BetaSimForm,####NEWBETA_6####
...
write.csv(Jura3Dat, file = "12_Jura3Dat.csv")
